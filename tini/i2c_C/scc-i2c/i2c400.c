/* This implemenation is based on an example I once grabbed from 
 ...
 Wed Feb  4 14:21:34 CET 2004 RP/LGM 
 	- port for TINIs400
	
rm *asm *ihx *hex *rel *lst *rst *lnk *sym *map *mem *lib	
../../sdcc/bin/sdcc -mds400 --model-flat24 --stack-10bit i2c400.c -c
../../sdcc/bin/sdcclib -a myi2clib.lib i2c400.rel	  
 */

#define DEBUG_I2C
// ==> DON'T DO THIS IS A LIBRARY <==
...
char i2c_recv(char addr, char count)
{
  ...
  if (ByteOutI2C(byte_in))
    {
  	printf("+1\n\r");
      if (i2cError == I2CERR_NAK) {printf("--1\n\r");
      I2CStop();printf("apres stop\n\r");
      }
      return i2cError;
    }
  printf("2\n\r");
