 ...
 * $History$
 *
 * Wed May 21 15:25:36 CEST 2003 RP
 *	- release for WSDL
 *	- Tango2
 ...
 *	
 * USAGE :
 export CLASSPATH=/home/raph/jwsdp-1.1/jwsdp-shared/lib/activation.jar:/home/raph/jwsdp-1.1/jwsdp-shared/lib/mail.jar:/home/raph/soap-2_3_1/lib/soap.jar:.
...
public class Soap2Tango2Client
	extends JFrame 
...
tf1=new JTextField("lgm/screen/1");
tf2=new JTextField("10");
tf3=new JTextField("http://localhost:8080/tango2-jaxrpc/tango2");
...
call.setTargetObjectURI("http://lgm.ac-grenoble.fr/wsdl/Tango2");
...
try {
if (ev.getActionCommand()=="TEST"){
	/*call.setMethodName("getserverlist");
	...
	call.setMethodName("commandinout");
	...
	params.addElement(new Parameter("String_1", String.class, "lgm/screen/1", Constants.NS_URI_SOAP_ENC));
	params.addElement(new Parameter("String_2", String.class, "SetText", Constants.NS_URI_SOAP_ENC));
	params.addElement(new Parameter("String_3", String.class, tf2.getText(), Constants.NS_URI_SOAP_ENC));
	call.setParams(params);
      	resp = call.invoke(new URL(tf3.getText()), "");
	if (!resp.generatedFault()) {
		//display return values
...
if (ev.getActionCommand()=="command_list_query") {
	...
	call.setMethodName("commandlistquery");
	call.setParams(params);
      	resp = call.invoke(new URL(tf3.getText()), "");
	if (!resp.generatedFault()) {
		//build choice bar
