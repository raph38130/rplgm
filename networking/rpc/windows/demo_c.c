...
static const RPC_CLIENT_INTERFACE LGM___RpcClientInterface =
    {
...
RPC_IF_HANDLE LGM_v1_0_c_ifspec = (RPC_IF_HANDLE)& LGM___RpcClientInterface;

extern const MIDL_STUB_DESC LGM_StubDesc;

static RPC_BINDING_HANDLE LGM__MIDL_AutoBindHandle;
...
LGM_malloc_wrapper( size_t _Size )
{
    return( malloc( _Size ) );
}

static void  __RPC_USER
LGM_free_wrapper( void * _p )
{
    free( _p );
}
