/*
 * 07 / 98 : BUG overflow corrigé
 */
#include "mem.h"

//arbre de descripteur pour initialiser le noyau
DESCR *ArbreS;
//le tableau des pages de tables
...
if (trouve)
   ...
   tdesc_ptp[i].valid = 1;
   tdesc_ptp[i].adr = (unsigned int) PageAlloc(SRAM);
...
for (iniv1 = iniv1_deb ; iniv1 <= iniv1_fin; iniv1++)
   ...
   */
   if (((iniv1 + 1) * 0x02000000) == 0)
      iniv2_fin = ((unsigned int) adr_fin >> 18) & 0x7f;
   else if (adr_fin >= (char *) ((iniv1 + 1) * 0x02000000))
      iniv2_fin = 127;
   else iniv2_fin = ((unsigned int) adr_fin >> 18) & 0x7f;
	...
	*/
      if ((iniv1 * 0x02000000 + (iniv2 + 1) * 0x40000) == 0)
         iniv3_fin = ((unsigned int) adr_fin >> 12) & 0x3f;
      else if (adr_fin >= (char *) (iniv1 * 0x02000000 + (iniv2 + 1) * 0x40000))
         iniv3_fin = 63;
      else iniv3_fin = ((unsigned int) adr_fin >> 12) & 0x3f;
...
print("A");
//code e2s 4 pages + 162 Bug
MapEspaceVirtuel(ArbreS,
		(char*)0x0000,
